...
### PASO 5: Remontar desde cero lo generado por disko ###
echo "🔁 Remontando subvolúmenes generados por disko..."
...
for SUBVOL in nix persist persist/etc-nixos persist/var persist/home persist/home/$USER; do
  mkdir -p "/mnt/${SUBVOL}"
  mount -o subvol=/${SUBVOL},compress=zstd,noatime /dev/disk/by-partlabel/disk-main-root "/mnt/${SUBVOL}"
done
...
### PASO 6: Re-clonar flake dentro de /mnt/etc/nixos ###
...
### PASO 8: Activación manual del sistema con nixos-enter ###
